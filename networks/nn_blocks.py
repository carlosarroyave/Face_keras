...
def conv_block(input_tensor, f, use_norm=False, k=3, strides=2):
    ...
    x = Conv2D(f, kernel_size=k, strides=strides, kernel_regularizer=tf.keras.regularizers.L2(w_l2),  
...
def embddding_fc_block(input_tensor):
    ...
    x = Dense(256, kernel_regularizer=tf.keras.regularizers.L2(w_l2))(x)
...
def upscale_nn(input_tensor, f, use_norm=True, w_l2=w_l2, norm=norm):
    x = input_tensor
    x = UpSampling2D()(x)
    x = Conv2D(f, kernel_size=4, kernel_regularizer=tf.keras.regularizers.L2(w_l2), 
...
def resize_tensor(inp, shape):
    if isinstance(shape, int):
        return Lambda(lambda x: tf.compat.v1.image.resize(x, [shape, shape]))(inp)
    elif isinstance(shape, list):
        return Lambda(lambda x: tf.compat.v1.image.resize(x, [shape[0], shape[1]]))(inp)
